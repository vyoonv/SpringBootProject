...
memberEmail.addEventListener("input", e => {
    ...
    if(inputEmail.trim().length === 0){
        ...
        // 잘 못 입력한 띄어쓰기가 있을 경우 없앰
        memberEmail.value = "";

        return;
    }

    // 4) 입력된 이메일이 있을 경우 정규식 검사 (알맞은 형태로 작성했는지 검사)
...
sendAuthKeyBtn.addEventListener("click", () => {
    ...
    // 화면에서는 타이머 시작하기
    authKeyMessage.innerText = initTime; //05:00세팅
...
// -> 입력된 인증번호와 발급된 인증번호가 같은지 비교 같으면 1, 아니면 0 반환
...
const checkPw = () =>{
    ...
    checkObj.memberPwConfirm = false; //비밀번호 확인false
}
...
// 3) 닉네임을 입력한 경우 정규식 검사
    const regExp = /^[가-힣\w\d]{2,10}$/;

// 4) 닉네임이 유효하지 않는 경우
    if(!regExp.test(memberNickname.value)){
        nickMessage.innerText = "유효하지 않는 닉네임입니다."
        nickMessage.classList.add("error");
        nickMessage.classList.remove("confirm");
        checkObj.memberNickname = false;
        return;
...
// 6) 유효한 경우 중복 검사 
    fetch("/member/checkNickname?memberNickkname=" +inputNickname)
    .then(resp => resp.text())
    .then(result => {
