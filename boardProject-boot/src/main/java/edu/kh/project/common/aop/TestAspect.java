...
@Component // bean 등록
//@Aspect // 공통된 관심사가 작성된 클래스임을 명시 (AOP 동작용 클래스)
@Slf4j // log 를 찍을 수 있는 객체(Logger) 생성 코드 추가 (Lombok 제공) 
public class TestAspect {
	...
	// <Pointcut 작성 방법>
	// execution( [접근 제한자]리턴타입 클래스명 메서드명 ([파라미터]) )
	// *클래스명은 패키지명부터 모두 작성 
	
	// 주요 어노테이션 
	// - @Aspect : Aspect를 정의하는데 사용하는 어노테이션으로, 클래스 상단에 작성함 
	...
	// * : 모든 리턴 타입을 나타냄 
	// edu.kh.project : 패키지명 
	// edu.kh.prohect : 패키지명을 나타냄, 패키지와 하위 패키지에 속하는 것을 대상으로 함 
	// .. : 0개 이상의 하위 패키지를 나타냄 
	// *Controller* : 이름에 Controller라는 문자열을 포함하는 모든 클래스를 대상으로 함 
	// .* : 모든 메서드를 나타냄 
	// * : 모든 
	// (..) : 0개 이상의 파라미터를 나타냄 
